...
func main() {
	...
	relayClient := masqueH2.NewClient(config)
	err = relayClient.ConnectToProxy()
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, port := range portListUDP {
		...
		go func(port int, l net.PacketConn) {
			...
			for {
				...
				if err != nil {
					if ok {
						logger.Error("Couldn't write to masque connection, DatagramTooLargeError", "err", err, "n", n)
					} else {
...
func HandleConnectMasque(relayClient *masqueH2.Client, c net.Conn, target string, isTcp bool, fail connFailFunc) (io.ReadWriteCloser, error) {
